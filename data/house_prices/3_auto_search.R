...
df$highvalue <- ifelse(df$SalePrice > 300000, 1, 0)
boxplot(df$LotFrontageImp ~ df$highvalue)
boxplot(df$GarageCars ~ df$highvalue)

df$threegarage <- ifelse(df$GarageCars ==3 , 1 , 0)
